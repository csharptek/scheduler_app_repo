...
function ScheduleInterviw(props) {
  ...
  const updateFunc = (data) => {
    ...
    let config = {
      method: "put",
      maxBodyLength: Infinity,
      url: `https://###########################################/schedulerAPI/interviews/${inter_id?.pk}/`,
      headers: {
  ...
  const handleSubmit = (e) => {
    ...
    let data = JSON.stringify({
      ...
      meeting_date_time: date_time_func(
    ...
    let config = {
      method: "post",
      maxBodyLength: Infinity,
      url: "https://#######################################schedulerAPI/interviews/create/",
      headers: {
        "Content-Type": "application/json",
      },
      data: data,
    ...
    axios
      ...
      .then((response) => {
        ...
        create_sendChat(
          ...
          date_time_func(formData.MeetingDate, formData.MeetingTime),
  ...
  const create_sendChat = async (can_id, dt, mlink) => {
    ...
    let config = {
      method: "get",
      maxBodyLength: Infinity,
      url: "https://##########################################/schedulerAPI/interviews/token/",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
      },
    };
  ...
  const sendMessage = async (tkn, can_id, dt, mlink) => {
    ...
    const endDateTime = new Date(
      startDateTime.getTime() + 0.5 * 60 * 60 * 1000
    ); // 30 mins duration
              ...
              <Col md={4}>
                <FormGroup>
                  ...
                  <FormControl
                    ...
                    required
                  >
                    <option value="0" key={"Select Candidate"}>
                      Select Candidate
                    </option>
                    {(userList || []).map((ul) => (
                      <option value={ul.id} key={ul.id}>
                        {ul.displayName + " || " + ul.userPrincipalName}
                      </option>
                    ))}
                  </FormControl>
                </FormGroup>
